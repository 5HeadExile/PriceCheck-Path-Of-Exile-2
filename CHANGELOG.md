...
### Added
- **Авто-масштаб ценников** под любое разрешение монитора: размер плашек выводится
  из реальной высоты текста награды (OCR), без ручных настроек. Доп. множитель
  `PriceChipScale` в конфиге.
- **Режим скриншота** (хоткей `F6`): временно показывает плашки в захвате экрана
  (чтобы попали в скриншот) и замораживает детектор — без мерцания. Виден индикатор.
- **Иконка приложения** (exe + трей).
- Пользовательский DPI-множитель UI меню/настроек (`UiScale` в конфиге).

### Changed
- **Стабильность оценки.** Много-проходный OCR (несколько порогов бинаризации) —
  панели разной яркости (полупрозрачная книга поверх разного фона) читаются
  надёжно. Относительный порог fuzzy-матчинга (≈30% длины имени) — меньше
  фантомных совпадений из шума. Мягкий confirm-gate: панель показывается при ≥1
  награде с ценой или ≥2 правдоподобных — одна потерянная строка не прячет всё.
- Производительность: исправлен рост списка порогов OCR в конфиге (×N проходов),
  меньше лишних пересканов от мерцания фона, быстрее скрытие при закрытии панели.
- Текст в кнопках без иконки («Отмена»/«Сохранить») — по центру.

### Fixed
- Меньше лишних «?»: усилён фильтр имён (мусор от строк рун-иконок отсекается).
- Самовосстановление при недоступных ценах poe.ninja: пустой результат больше не
  «замораживается», монитор ретраит, пока цены не подтянутся (без перезапуска).
...
### Changed
- Количество наград: надёжный разбор «Nx» (1x/2x/3x, в т.ч. слитно «3xFoo»);
  оверлей показывает сумму и цену за штуку в скобках для стака >1. Защита от
  ложного захвата числа из «(Level 19)».
- Теги наград: снятие префикса «Skill:/Support:» перед поиском цены; скиллы/
  саппорты, которых нет на poe.ninja, распознаются по тегу и показываются как «?».
